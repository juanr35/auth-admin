...
export default function AuthForm({ 
  ...
  useEffect(() => {    
    if (status === "authenticated") {
      push(`${process.env.NEXT_PUBLIC_ADMIN_AUTH_DOMAIN}/profile`)
    }
  }, [status])
  ...
  */
  ...
  const handleSubmit = async (e) => {
    ...
    if (typeAuth == "signin") {
      ...
      try {        
        res = await axios({
          url: `${process.env.NEXT_PUBLIC_ADMIN_AUTH_DOMAIN}/api/auth/users/signup`,
          method: "post",
          data: fields
        })

        if (res.status == 201) {
          push(`${process.env.NEXT_PUBLIC_ADMIN_AUTH_DOMAIN}/auth/verify/${res.data._id}`)
        }
        ...
        <Card className="bg-secondary shadow border-0">
          <CardBody className="px-lg-5 pt-lg-5">
            <div className="text-center text-muted mb-4">
              <small>{typeAuth == "signup" ? "Nuevo usuario" : "Iniciar Session"}</small>
            </div>
            <Form role="form" onSubmit={handleSubmit}>
              ...
              <div className="text-center">
                <Button 
                  ...
                  type="submit"
                >
                  { typeAuth == "signup" ? "Nueva cuenta" : "Entrar"}
                </Button>
              </div>
            </Form>
          </CardBody>
          ...
          (
            <Row className="mt-3">
              <Col className="text-center" xs="12">
                <a
                  ...
                  href={`${process.env.NEXT_PUBLIC_ADMIN_AUTH_DOMAIN}/auth/signin`}
                >
                  <small>¿Ya tienes una cuenta? Inicia Session</small>
                </a>
              </Col>
            </Row>
          ) : (
            <Row className="mt-3">
              <Col xs="6">
                <a
                  className="text-light"
                  href={`${process.env.NEXT_PUBLIC_ADMIN_AUTH_DOMAIN}/auth/forgotPassword`}
                >
              ...
              <Col className="text-right" xs="6">
                <a
                  className="text-light"
                  href={`${process.env.NEXT_PUBLIC_ADMIN_AUTH_DOMAIN}/auth/new-user`}
                >
