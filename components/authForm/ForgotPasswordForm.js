...
const stepsMessages= [
  { 
    title: 'Ingrese la direccion de correo electronico', 
    name: 'email',
    placeholder: 'example@mail.com',
    error: 'Enter the email' 
  },
  { 
    title: 'Se ha enviado un codigo secreto a su correo', 
    name: 'code',
    placeholder: 'Codigo', 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      switch (step) {
        case 0:
          res = await axios({
            url: `${process.env.NEXT_PUBLIC_ADMIN_AUTH_DOMAIN}/api/auth/users/forgot-password/${fields.email}`,
            method: "get",
          })              
          break;
        case 1:
      ...
      if (res.status == 201) {
        if ( step === 2 ) {
          push(`${process.env.NEXT_PUBLIC_ADMIN_AUTH_DOMAIN}/auth/signin`)
        }
