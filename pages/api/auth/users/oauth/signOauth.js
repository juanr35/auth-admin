...
export default async function handler(req, res) {
  ...
  try {
    ...
    let user = await findOneMongoDriver(client, "admins", { 
    ...
    await session.withTransaction(async () => {
      ...
      let _id
      user = await findOneMongoDriver(client, "admins", { 
        email: req.body.email,
        type: 'credential'
      })
      if (user) {
      ...
      const newUser = await createDocumentMongoDriver(client, "admins", contentDoc, session);
      assert.ok( newUser );
