  ...
  providers: [
      ...
      async authorize(credentials, req) {
        try {        
          const res = await axios({
            url: `${process.env.NEXT_PUBLIC_ADMIN_AUTH_DOMAIN}/api/auth/users/signin`,
            method: "post",
            data: {...credentials}
          })
          return res.data
    ...
    Credentials({
      ...
      async authorize(credentials, req) {
        try {        
          const res = await axios({
            url: `${process.env.NEXT_PUBLIC_ADMIN_AUTH_DOMAIN}/api/auth/users/verify`,
            method: "post",
            data: {...credentials}
          })
          return res.data
  ...
  callbacks: {
    ...
    async jwt({ token, user, account, profile, isNewUser }) {
      ...
      if (account?.type === 'oauth') {       
        try {        
          const res = await axios({
            url: `${process.env.NEXT_PUBLIC_ADMIN_AUTH_DOMAIN}/api/auth/users/oauth/signOauth`,
            method: "post",
            data: profile
          })
